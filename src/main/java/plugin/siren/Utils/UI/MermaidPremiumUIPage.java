...
public class MermaidPremiumUIPage extends InteractiveCustomUIPage<MermaidPremiumUIPage.MermaidUIEventData> {
    ...
    private MermaidPremiumUIPage.Category selectedCategory = Category.PRESETS;

    public MermaidPremiumUIPage(@Nonnull PlayerRef playerRef, MermaidPremiumUIPage.Category category) {
        this(playerRef);
        this.selectedCategory = category;
    }

    public MermaidPremiumUIPage(@Nonnull PlayerRef playerRef) {
        super(playerRef, CustomPageLifetime.CanDismiss, MermaidPremiumUIPage.MermaidUIEventData.CODEC);
    }
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull MermaidPremiumUIPage.MermaidUIEventData data) {
        Player player = store.getComponent(ref, Player.getComponentType());
        ...
        if(mermaid != null && mermaidSettings != null) {
            ...
            if (data.category != null) {
                MermaidPremiumUIPage.Category newCategory = MermaidPremiumUIPage.Category.fromId(data.category);
                if (newCategory != this.selectedCategory) { /// //////////////////////////////// REDO THIS STUPID MATH
                ...
                MermaidPremiumUIPage merPage = new MermaidPremiumUIPage(playerRef, selectedCategory);
    ...
    private void buildCategoryList(@Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, MermaidSettingsComponent mermaidSettings) {
        ...
        int j = 0;
        for (int i = 0; i < MermaidPremiumUIPage.Category.values().length; i++) {
            MermaidPremiumUIPage.Category category = MermaidPremiumUIPage.Category.values()[i];
            ...
            if(mermaidSettings.getMermaidTailId().getValue() == MermaidModel.Mammal.getValue() && category == Category.COLOR_STYLE){
                j++;
                continue;
            }
            if(mermaidSettings.getMermaidTailId().getValue() == MermaidModel.Mermaid.getValue() && category != Category.fromId(mermaidSettings.getCategory()) &&
                    (category == Category.COLOR || category == Category.SHINY_FABRIC_COLOR || category == Category.FANTASY_COTTON_COLOR)){
                j++;
                continue;
            }
    ...
    private void displayCategory(@Nonnull MermaidPremiumUIPage.Category category, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, MermaidSettingsComponent mermaidSettings, Player player) {
        ...
        } else if (category.id.equalsIgnoreCase("color_style")) {
            for (int i = 0; i < category.getTailSelectionCount(); i++) {
                String selector = "#CategoryContent #ColorStyle" + i + " #Selector";
                eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, selector, EventData.of("ColorStyle", String.valueOf(i)));
            }
        ...
        } else if (category.id.equalsIgnoreCase("shiny_fabric_color")) {
            for (int i = 0; i < category.getTailSelectionCount(); i++) {
                String selector = "#CategoryContent #TailColor" + i + " #Selector";
                eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, selector, EventData.of("ShinyFabric", String.valueOf(i)));
            }
        } else if (category.id.equalsIgnoreCase("fantasy_cotton_color")) {
            for (int i = 0; i < category.getTailSelectionCount(); i++) {
                String selector = "#CategoryContent #TailColor" + i + " #Selector";
                eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, selector, EventData.of("FantasyCotton", String.valueOf(i)));
            }
    ...
    private static enum Category {
        ...
        COLOR_STYLE(
                ...
                "Pages/MermaidUI/Categories/Premium/ColorStyleContent.ui",
                2
        ),
        COLOR(
                "color",
        ...
        SHINY_FABRIC_COLOR(
                "shiny_fabric_color",
                "server.customUI.mermaids.mermaidui.category.color.shinyFabric",
                "Pages/MermaidUI/Categories/Premium/ShinyFabricColorContent.ui",
                14
        ),
        FANTASY_COTTON_COLOR(
                "fantasy_cotton_color",
                "server.customUI.mermaids.mermaidui.category.color.fantasyCotton",
                "Pages/MermaidUI/Categories/Premium/FantasyCottonColorContent.ui",
                13
        ),
        DORSAL_FIN(
                "dorsal_fin",
        ...
        public static MermaidPremiumUIPage.Category fromId(String id) {
            for (MermaidPremiumUIPage.Category category : values()) {
    ...
    public static class MermaidUIEventData {
        ...
        static final String KEY_TAIL_COLOR_STYLE = "ColorStyle";
        static final String KEY_TAIL_COLOR = "TailColor";
        static final String KEY_OCEAN_FLUKE_COLOR = "OceanFlukeColor";
        static final String KEY_SHINY_FABRIC_COLOR = "ShinyFabricColor";
        static final String KEY_FANTASY_COTTON_COLOR = "FantasyCottonColor";
        ...
        public static final BuilderCodec<MermaidUIEventData> CODEC = BuilderCodec.builder(
                        MermaidPremiumUIPage.MermaidUIEventData.class, MermaidPremiumUIPage.MermaidUIEventData::new
                )
                ...
                .addField(new KeyedCodec<>(KEY_TAIL_COLOR_STYLE, Codec.STRING), (entry, s) -> entry.tailColorStyle = s, entry -> entry.tailColorStyle)
                ...
                .addField(new KeyedCodec<>(KEY_SHINY_FABRIC_COLOR, Codec.STRING), (entry, s) -> entry.shinyFabricColor = s, entry -> entry.shinyFabricColor)
                .addField(new KeyedCodec<>(KEY_FANTASY_COTTON_COLOR, Codec.STRING), (entry, s) -> entry.fantasyCottonColor = s, entry -> entry.fantasyCottonColor)
        ...
        private String tailColorStyle;
        private String tailColor;
        private String oceanFlukeColor;
        private String shinyFabricColor;
        private String fantasyCottonColor;
