package plugin.siren.Systems.NPCs;
...
public class MermaidNPCSystem extends EntityTickingSystem<EntityStore> {
    private final ComponentType<EntityStore, MermaidNPCComponent> mermaidNPCComponentType;

    public MermaidNPCSystem(ComponentType<EntityStore, MermaidNPCComponent> mermaidNPCComponentType){
        this.mermaidNPCComponentType = mermaidNPCComponentType;
    }
    ...
    public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk,
        ...
        if(ref == null || !ref.isValid()){
            Mermaids.LOGGER.atFine().log("Failed to get reference : MermaidNPCSystem");
        }else {
            MermaidNPCComponent mermaidNPC = archetypeChunk.getComponent(index, mermaidNPCComponentType);

            NPCEntity entity = commandBuffer.getComponent(ref, Objects.requireNonNull(NPCEntity.getComponentType()));
                            ...
                            if (transform != null) {
                                ...
                                if (Mermaids.getConfig().get().getBlockTransformation()) {
                                    ...
                                    if (footBlockId.equals("Alchemy_Cauldron_Big")) {//footBlockId == 24){//Large Cauldron
                                        if(!mermaidNPC.getH2OBlock().get()) {
                                            mermaidNPC.setH2OBlock(true);
                                        }
                                    ...
                                    if (!h2o && mermaidNPC.getH2OBlock().get()) {
                                        mermaidNPC.setH2OBlock(false);
                                    }
                                }
                                ...
                                if (Mermaids.getConfig().get().getRainTransformation()) {
                                            ...
                                            if(weatherTracker == null){
                                                ...
                                                if( weatherNaturalAsset != null){
                                                    ...
                                                    if (weatherNaturalId.equalsIgnoreCase("zone1_rain") || weatherNaturalId.equalsIgnoreCase("zone1_swamp_rain") || //Zone 1
                                                        ...
                                                        if(!mermaidNPC.getRainTransform().get()){
                                                            mermaidNPC.setRainTransform(true);
                                                        }
                                                        raining = true;
                                                    }
                                    ...
                                    if (!raining && mermaidNPC.getRainTransform().get()) {
                                        mermaidNPC.setRainTransform(false);
                                    }
                                }
                                ...
                                if(footFluidId > 0 || bodyFluidID > 0 || belowFootFluidID > 0){
                                    mermaidNPC.setInFluidBlock(true);
                                }

                                if(footFluidId == 0 && bodyFluidID == 0 && belowFootFluidID == 0 && mermaidNPC.isInFluidBlock()){
                                    mermaidNPC.setInFluidBlock(false);
                                }
                            }
                        }
                    });
            ...
            boolean transformPermission = true;//PermissionsModule.get().hasPermission(playerRef.getUuid(), "mermaids.transform") || !Mermaids.getConfig().get().getRequireTransformationPermission();
            boolean toggleMermaid = true;//mermaidSettings.getToggleMermaid();
            ...
            boolean h2OorRain = mermaidNPC.getH2OBlock().get() || mermaidNPC.getRainTransform().get();
            boolean inFluidBlock = mermaidNPC.isInFluidBlock();

            boolean permMerPotion = false;//(transModeZero || transModeOne) && mermaidSettings.ifPermanentPotion();
            boolean mermaidPotionEffect = false;//mermaidNPC.isPotionEffectTransformation();
            ...
            boolean forcedMermaid = false;//requireForcedMermaid && mermaidSettings.isForcedMermaid() && !Mermaids.getConfig().get().ifForceMermaidOnlyInWater();
            boolean forcedMermaidWater = false;//requireForcedMermaid && mermaidSettings.isForcedMermaid() && Mermaids.getConfig().get().ifForceMermaidOnlyInWater();
            ...
            if (((((movementStatesTransform || h2OorRain || inFluidBlock) && (transModeZero || permMerPotion || mermaidPendant)) || mermaidPotionEffect || (mermaidOnLand && (transModeZero || permMerPotion || mermaidPendant)))
                ...
                if (!mermaidNPC.isUnderwater()) {
                    mermaidNPC.setUnderwater(true);
                    mermaidNPC.setElapsedTime(0f);

                    if (Mermaids.ifDebug()) {
                        Mermaids.LOGGER.atInfo().log("Entity entered water");
                    }
                }

                if (mermaidNPC.getElapsedTime() < 500f) {
                    mermaidNPC.incrementTick();
                }

                if(mermaidNPC.getPotionElapsedTime() > 0){
                    mermaidNPC.decrementPotionTick();
                }

                //Began transformation into Mermaid
                if ((!mermaidNPC.isMermaid() && mermaidNPC.getElapsedTime() >= 35f) || (!mermaidNPC.isMermaid() && forcedMermaid)) {
                    if (Mermaids.ifDebug()) {
                        Mermaids.LOGGER.atInfo().log("now swimming (tick 35)");
                    }

                    if (mermaidNPC != null) {
                        ModelComponent modelComponent = commandBuffer.getComponent(ref, ModelComponent.getComponentType());
                        if (modelComponent != null) {
                            ModelComponent model = (ModelComponent) modelComponent.clone();
                            mermaidNPC.setModelComponent(model);

                            if (Mermaids.ifDebug()) {
                                Mermaids.LOGGER.atInfo().log("set the model component");
                            }
                        } else {
                            Mermaids.LOGGER.atInfo().log("MermaidNPCSystem: modelComp == null");
                        }

                        if (Mermaids.ifDebug()) {
                            Mermaids.LOGGER.atInfo().log("MermaidNPCSystem: replacing entity model");
                        }

                        String mermaidTailModel = "Mermaids_Zombie_Mermaid";

                        ModelAsset modelAsset = ModelAsset.getAssetMap().getAsset(mermaidTailModel);
                        if (modelAsset == null) {
                            Mermaids.LOGGER.atSevere().log("entity had an error of getting the Mermaid Model. Error: WaterSystem: " + mermaidTailModel + " Model not found.");
                        } else {
                            Model model = Model.createUnitScaleModel(modelAsset);
                            commandBuffer.replaceComponent(ref, ModelComponent.getComponentType(), new ModelComponent(model));
                        }
                    } else {
                        Mermaids.LOGGER.atSevere().log("entity had an error of getting the Mermaid Component. Error: WaterSystem: Mermaid Component == null [movementStates]");
                    }

                    mermaidNPC.setArmorElapsedTime(0f);
                    mermaidNPC.setMermaid(true);
                    ...
                    if(Mermaids.getConfig().get().ifConsoleLogs()) {
                        Mermaids.LOGGER.atInfo().log( "ENTITY has transformed into a Mermaid.");
                    }
                }
    ...
    public Query<EntityStore> getQuery(){
        return Query.and(this.mermaidNPCComponentType);
    }
}
