...
public class ComponentFixCmd extends AbstractPlayerCommand {
    public ComponentFixCmd() {
        super("compfix", "server.commands.mermaids.bug.componentFix.desc");

        this.addAliases("fix");
        this.addAliases("componentfix");
        this.addAliases("componentsfix");
    ...
    protected void execute(@Nonnull CommandContext commandContext, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        world.execute(() -> {
            ...
            if (merComp == null) {
                ...
                if(Mermaids.getConfig().get().ifConsoleLogs()) {
                    Mermaids.LOGGER.atInfo().log(player.getDisplayName() + " tried to receive Mermaid Component in the Mermaids Bug CompFix Cmd but already has it.");
                }
            }
            ...
            if (merSett == null) {
                ...
                if(Mermaids.getConfig().get().ifConsoleLogs()) {
                    Mermaids.LOGGER.atInfo().log(player.getDisplayName() + " tried to receive Mermaid Settings Component in the Mermaids Bug CompFix Cmd but already has it.");
                }
            }
        });
    }
