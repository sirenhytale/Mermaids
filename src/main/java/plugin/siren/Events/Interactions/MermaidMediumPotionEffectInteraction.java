...
public class MermaidMediumPotionEffectInteraction extends ApplyEffectInteraction {

    public static final BuilderCodec<MermaidMediumPotionEffectInteraction> CODEC = BuilderCodec
            .builder(MermaidMediumPotionEffectInteraction.class, MermaidMediumPotionEffectInteraction::new, ApplyEffectInteraction.CODEC)
            .documentation("Adds the Mermaid Transformation Medium potion for the player.")
            .build();
    ...
    protected void firstRun(@Nonnull InteractionType interactionType, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
        ...
        if (ref == null || !ref.isValid()) {
            Mermaids.get().getLogger().atFine().log("In MermaidMediumPotionEffectInteraction, failed to get reference.");
            return;
        }

        CommandBuffer<EntityStore> buffer = context.getCommandBuffer();
        if (buffer == null) {
            Mermaids.get().getLogger().atFine().log("In MermaidMediumPotionEffectInteraction, failed to get buffer.");
            return;
        }

        Player player = buffer.getComponent(ref, Player.getComponentType());
        if (player == null) {
            Mermaids.get().getLogger().atFine().log("In MermaidMediumPotionEffectInteraction, failed to get player.");
            return;
        }
        ...
        if(Mermaids.ifDebug()){
            player.sendMessage(Message.raw("You have drank the medium potion of Mermaid Transformation."));
        }

        float elapsedTime = mermaid.getPotionElapsedTime() / 20f;
        mermaid.addPotionElapsedTime(20f * 600f);

        long scheduleDelay = Math.round(elapsedTime) + 600;

        World world = player.getWorld();
        HytaleServer.SCHEDULED_EXECUTOR.schedule(() -> world.execute(() -> {
            if(ref == null || !ref.isValid()){
                Mermaids.get().getLogger().atFine().log("In MermaidMediumPotionEffectInteraction, failed to get reference in world execute.");
            }else{
